...
--
-- Deltas vs PostgreSQL: BYTEA -> BLOB, timestamps stored as INTEGER epoch-millis (see
-- InstantEpochMillisConverter; xerial's getTimestamp can't parse Hibernate's epoch text),
-- octet_length() -> length(). The logical schema is identical. Migrations only add structure.
